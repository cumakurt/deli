...
def _compute_active_users_for_scenario(
    ...
    if scenario == "spike":
        ...
        spike_start = ramp + max(
            0, (config.duration_seconds - config.spike_duration_seconds * 2) / 2
        )
...
async def run_scenario(
    ...
    peak_users = (
        config.users + config.spike_users if config.scenario.value == "spike" else config.users
    )
    max_in_flight = min(
        MAX_IN_FLIGHT_LIMIT, max(peak_users * MAX_IN_FLIGHT_MULTIPLIER, MIN_IN_FLIGHT_LIMIT)
    )
    semaphore = asyncio.Semaphore(max_in_flight)

    async with await create_client(http2=http2, timeout=DEFAULT_HTTP_TIMEOUT) as client:
        ...
        if config.scenario.value == "constant":
            ...
            if config.iterations > 0:
                _, pending = await asyncio.wait(workers, timeout=config.duration_seconds)
                if pending:
                    stop_event.set()
            ...
            while time.perf_counter() - start_time < config.duration_seconds:
                ...
                if (
                    config.iterations > 0
                    and started == config.users
                    and all(t.done() for t in workers)
                ):
                    break
            ...
            started = 0
            spike_started = 0
            spike_stop_event = asyncio.Event()
            spike_stop_requested = False
            while time.perf_counter() - start_time < config.duration_seconds:
                now = time.perf_counter() - start_time
                while started < config.users and now >= next_start:
                    t = asyncio.create_task(
                ...
                if now >= spike_end and spike_started > 0 and not spike_stop_requested:
                    spike_stop_event.set()
                    spike_stop_requested = True
                if (
                    config.iterations > 0
                    and started == config.users
                    and spike_started == config.spike_users
                    and all(t.done() for t in workers)
                ):
                    break
                await asyncio.sleep(RAMP_POLL_SEC)
