...
async def _run_phase_collector(
    num_users: int,
    duration_seconds: float,
    requests: list[ParsedRequest],
    think_time_ms: float,
) -> tuple[MetricsCollector, float, float]:
    ...
    Returns:
        Tuple of (collector, start_timestamp, end_timestamp)
    """
    if not requests:
        now = time.perf_counter()
        return MetricsCollector(max_results=PHASE_COLLECTOR_MAX_RESULTS), now, now
    ...
    return collector, start_ts, end_ts
...
async def run_phase(
    ...
    """Run one stress phase and return retained raw results for compatibility."""
    collector, start_ts, end_ts = await _run_phase_collector(
        num_users=num_users,
...
def _phase_result_from_aggregate(
    ...
    """Build StressPhaseResult from aggregate metrics and check thresholds."""
...
def _phase_metrics(
    ...
    """Build StressPhaseResult from retained phase results and check threshold."""
    start_ms = start_ts * 1000
    end_ms = end_ts * 1000
    agg = compute_aggregate(results, start_ms, end_ms)
    return _phase_result_from_aggregate(
        agg=agg,
...
def _phase_metrics_from_collector(
    ...
    """Build StressPhaseResult from full streaming collector metrics."""
    return _phase_result_from_aggregate(
        agg=collector.full_aggregate(),
        timeout_count=collector.timeout_count,
        phase=phase,
        users=users,
        duration_seconds=duration_seconds,
        config=config,
...
async def run_stress_test(
    ...
    if config.scenario == StressScenario.SPIKE_STRESS and config.spike_users > 0:
        ...
        collector, _, _ = await _run_phase_collector(
            config.spike_users, hold, requests, config.think_time_ms
        )
        pr = _phase_metrics_from_collector(collector, 0, config.spike_users, hold, config)
        phases.append(pr)
        ...
        pr = _phase_metrics_from_collector(
            collector, 0, config.soak_users, config.soak_duration_seconds, config
        )
        phases.append(pr)
        if pr.threshold_exceeded:
            max_sustainable = 0
    ...
    while current_users <= config.max_users:
        ...
        pr = _phase_metrics_from_collector(
            collector, phase_idx, current_users, config.step_interval_seconds, config
        )
        phases.append(pr)
    ...
    from .stress_report import (
        generate_stress_json_report,
        generate_stress_junit_report,
    ...
    if junit_path:
        generate_stress_junit_report(junit_path, result, config)
        if live:
            console.print(f"[dim]JUnit report:[/dim] {junit_path}")
    if json_path:
        generate_stress_json_report(json_path, result, config)
        if live:
