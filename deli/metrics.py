...
class MetricsCollector:
    ...
    def add(self, result: RequestResult) -> None:
        ...
        elif not self._overflow_warned and current_len >= int(
            self._max_results * OVERFLOW_WARNING_THRESHOLD_PCT
        ):
            logger.warning(
                ...
                current_len,
                self._max_results,
                100.0 * current_len / self._max_results,
            )
            self._overflow_warned = True
    ...
    def add_batch(self, results: Iterable[RequestResult]) -> None:
        ...
        if not self._overflow_warned and self._overflow_count > 0:
            logger.warning(
                "MetricsCollector buffer overflow: %d results dropped.", self._overflow_count
            )
            self._overflow_warned = True
